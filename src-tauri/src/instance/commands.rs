...
pub async fn create_instance(
  ...
  let path = directory.dir.join("versions").join(&name);
  match crate::instance::helpers::instance_creator::create_instance(
    app,
    directory,
    name,
    description,
  ...
  .await
  {
    Ok(_) => Ok(()),
    Err(e) => {
      if e != InstanceError::ConflictNameError.into() && path.exists() {
