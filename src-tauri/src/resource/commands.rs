...
pub async fn retrive_game_version_list() -> SJMCLResult<Vec<GameResourceInfo>> {
  let url = format!(
    ...
    get_download_api(ResourceType::Game)
  );
      ...
      if response.status().is_success() {
        match response.json::<Value>().await {
          Ok(data) => {
            ...
            Ok(game_version_list)
          }
          Err(_) => Err(ResourceError::ParseError.into()),
        }
      } else {
        Err(ResourceError::FetchError.into())
      }
    }
    Err(_) => Err(ResourceError::FetchError.into()),
  }
}
