...
impl TaskEvent {
  ...
  pub fn emit_started(app: &AppHandle, id: u32, total: i64) {
    TaskEvent {
      id,
  ...
  pub fn emit_failed(app: &AppHandle, id: u32, reason: String) {
    TaskEvent {
      id,
  ...
  pub fn emit_cancelled(app: &AppHandle, id: u32) {
    TaskEvent {
      id,
  ...
  pub fn emit_completed(app: &AppHandle, id: u32) {
    TaskEvent {
      id,
  ...
  pub fn emit_created(app: &AppHandle, id: u32) {
    TaskEvent {
      id,
...
impl TaskState {
  pub fn new(
    ...
    Self {
      ...
      current,
      total,
      filepath: state_dir.join(format!("states/{}.json", task_id)),
      task_param,
      monitor_state,
    }
  }
  ...
  pub fn save(&self) -> Result<(), std::io::Error> {
    ...
    std::fs::write(&self.filepath, json_string)
  }
  ...
  S: Stream<Item = T>,
{
  pub fn new(
    ...
    let current = task_state.current.clone();
    Self {
      app_handle,
      target,
